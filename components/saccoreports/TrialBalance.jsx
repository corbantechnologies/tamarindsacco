  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 max-w-5xl mx-auto overflow-hidden">
      <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
        ...
        <div className={`flex items-center gap-3 px-6 py-3 rounded-2xl border-2 transition-all duration-500 ${data.is_balanced ? 'bg-emerald-50 border-emerald-100' : 'bg-rose-50 border-rose-100 animate-pulse'}`}>
          {data.is_balanced ? (
            <>
        ...
        <table className="w-full border-collapse min-w-[700px]">
          <thead>
            <tr className="bg-gray-50/50">
              <th className="text-left py-4 px-8 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-100">Code</th>
              <th className="text-left py-4 px-8 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-100">Account Particulars</th>
              <th className="text-left py-4 px-8 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-100">Type</th>
              <th className="text-right py-4 px-8 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-100">Debit (DR)</th>
              <th className="text-right py-4 px-8 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-100">Credit (CR)</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {data.accounts.map((acc, idx) => (
              <tr key={acc.code} className="hover:bg-gray-50/50 transition-colors group">
                <td className="py-4 px-8 text-xs font-mono text-gray-400 group-hover:text-primary transition-colors">{acc.code}</td>
                ...
                <td className="py-4 px-8 text-right font-mono text-xs font-bold text-gray-800">
          ...
          <tfoot>
            <tr className="bg-primary/5">
              <td colSpan={3} className="py-6 px-8 text-xs font-black text-primary uppercase tracking-[0.3em] text-right">Trial Balance Totals</td>
              <td className="py-6 px-8 text-right">
                <div className="inline-block border-b-4 border-double border-primary pb-1">
                  <span className="text-lg font-black text-primary font-mono">{formatCurrency(data.total_debit)}</span>
                </div>
              </td>
              <td className="py-6 px-8 text-right">
                <div className="inline-block border-b-4 border-double border-primary pb-1">
                  <span className="text-lg font-black text-primary font-mono">{formatCurrency(data.total_credit)}</span>
                </div>
              </td>
            </tr>
          </tfoot>
